...
1. **Wrong `target_width`.** Pause the run video, measure how many pixels wide the animal is (together with anything it moves as one blob with, such as a beetle's ball), and put that in the `target_width` column. This is the single most important tracking parameter.
