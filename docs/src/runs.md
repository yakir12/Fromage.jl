...
| `fps` | video's frame rate | how many frames per second to track. Lower it for slow targets to speed things up. Cannot exceed the video's own frame rate. Tracking advances whole frames, so the only rates available are the video's rate divided by a whole number: ask for anything else and you get the nearest one (on 30 fps footage, `fps = 20` tracks at 15, and `fps = 25` at 30). The timestamps in your track file always describe the rate actually used, never the one requested — for a run split across several segments, the first segment's (see the segment notes below). |
...
- The segments are assumed to have been **filmed at the same frame rate** — they are pieces of one continuous recording, so normally they are. This is not checked: if you give segments with different frame rates an explicit `fps`, they give **wrong results silently** — each one is tracked at the nearest rate *its own* footage allows, the diagnostic video's playback speed is derived from the first segment, and, the one that matters, every timestamp in the track file is spaced at the first segment's rate, so the later segments' times are wrong and drift further the longer the run. Segments of 30 fps and 25 fps footage given `fps = 7` are tracked at 7.5 and 6.25 fps, and drift 0.35 s apart over a 4 s run. Leave `fps` blank and mismatched segments are caught, because the rate is then read from each video and the segments must agree on it.
