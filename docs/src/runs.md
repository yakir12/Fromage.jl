...
- **Segments cut from the same file must be in order and must not overlap**: each one's `start` at or after the previous one's `stop`. Starting exactly where the previous segment stopped is fine, and so is leaving a gap. This is checked: two windows of one file that overlap, repeat each other, or run backwards are reported and nothing is tracked until you fix them.
...
- A `native_fps` or `aspect` written on **any** row of a run is a statement about the whole recording, and applies to every one of its segments — so you write it once, on whichever row is convenient, and leave it blank on the rest. Two rows claiming *different* values cannot both be true and are rejected.
...
- **A run's rows must be consecutive**: no other run's row may sit between two rows of the same `run_id`. This is checked: a run whose rows are split is reported, before any video is opened, and nothing is tracked until you fix it.
