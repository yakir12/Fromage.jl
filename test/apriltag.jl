...
const HMILD = SMatrix{3, 3, Float64}(1.5, -0.03, 5.0e-6, 0.05, 1.5, 3.0e-6, 960.0, 540.0, 1.0)
const HHARSH = SMatrix{3, 3, Float64}(1.5, -0.05, 1.0e-4, 0.1, 1.4, 5.0e-5, 960.0, 540.0, 1.0)
const HMILD2 = SMatrix{3, 3, Float64}(1.4, 0.06, -4.0e-6, -0.08, 1.45, 4.0e-6, 900.0, 600.0, 1.0)
...
const ANGLES = [0.1, -0.2, 0.15, -0.05]
...
@testset "AprilTag geometry" begin
    ...
    @testset "homography_dlt recovers a known homography" begin
        ...
        @test maximum(norm(apply_h(H, pts[i]) - dst[i]) for i in eachindex(pts)) < 1.0e-8
    end
    ...
    @testset "fit_metric makes every tag a 96 cm square, jointly (not one tag)" begin
        ...
        @test norm(â - b̂) ≈ norm(a - b) rtol = 1.0e-4
    end
    ...
    @testset "registration is drone-motion invariant: one ground point, two frames" begin
        ...
        @test norm(cm1 - cm2) < 1.0e-4                                 # same cm despite drone move
        ...
        @test d̂ ≈ norm(g1 - g2) rtol = 1.0e-4
    end
    ...
    @testset "RegisteredWarp: the registered background stack is drone-motion invariant" begin
        ...
        @test all(maximum(abs, Float32.(stack[r, c, k]) .- Float32.(ground[r, c])) < 1.0e-6 for k in 1:3)
        bg = dropdims(maximum(Float32.(stack[r, c, :]), dims = 3), dims = 3)
        @test maximum(abs, bg .- Float32.(ground[r, c])) < 1.0e-6
        ...
        @test Float32(stack2[10, 12, 1]) ≈ Float32(ground[20, 24]) atol = 1.0e-6
        @test Float32(stack2[10, 12, 2]) ≈ Float32(ground[20, 24]) atol = 1.0e-6
    ...
    @testset "the diagnostic canvas is gauged by center/north, not by the tag fit" begin
        ...
        @test s1.gauge(apply_h(ref.M, centre_px)) ≈ SVector(0.0, 0.0) atol = 1.0e-9
        gn = s1.gauge(apply_h(ref.M, north_px))
        @test gn[1] < 0 && abs(gn[2]) < 1.0e-9
    ...
    @testset "without north the canvas is unchanged: raw cm, framed on the tags" begin
        ...
        was(p) = (
            c = apply_h(ref.M, p);
            CartesianIndex(
                round(Int, (c[2] - yc) * ppc + DIAGNOSTIC_SIZE / 2),
