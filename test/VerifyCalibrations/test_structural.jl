@testset "structural" begin
    @testset "duplicate calibration_id" begin
        # same id, but otherwise different (different extrinsic) so it is NOT also a duplicate calibration.
    ...
    @testset "duplicate calibration (identical rows)" begin
        ...
        @test flagged(df, 2, "duplicate calibration")
        @test !flagged(df, 2, "same calibration with conflicting parameters")
    end
    ...
    @testset "video duplicate with conflicting non-identity params" begin
        ...
        # checker_size: still the same calibration, so the 2nd is a duplicate; and because a
        ...
        @test flagged(df, 2, "duplicate calibration")
        @test flagged(df, 2, "same calibration with conflicting parameters")
        @test !flagged(df, 1, "duplicate calibration")
        @test !flagged(df, 1, "same calibration with conflicting parameters")
    end
    ...
    @testset "video NOT a duplicate when an identity field differs" begin
        # center is part of the identity key, so differing center -> distinct calibrations, no flags.
    ...
    @testset "only_scale duplicate = identical except id/issues" begin
        ...
        @test flagged(dup, 2, "duplicate calibration")
        @test !flagged(dup, 1, "duplicate calibration")
        diff = check("s_diffscale.csv", [scalerow(calibration_id = "s1", scale = 9.5),
                                         scalerow(calibration_id = "s2", scale = 10.5)])
        @test !flagged(diff, 2, "duplicate calibration")
    end
    ...
    @testset "duplicate calibration (same file via different path spelling)" begin
        ...
        # the duplicate-calibration check; verify_unique_calibrations! realpaths the key columns
    ...
    @testset "no false duplicate from rows nulled by earlier checks" begin
        ...
        # skips rows that already carry issues, so neither gets a spurious "duplicate calibration".
