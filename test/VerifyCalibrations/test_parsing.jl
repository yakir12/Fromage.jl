@testset "parsing" begin
    ...
    @testset "low-level parsers" begin
        @testset "MyTemporal: seconds vs HH:MM:SS precedence" begin
            @test VC.mytryparse(VC.MyTemporal, "1.5")      == 1.5    # float path taken before Time
            @test VC.mytryparse(VC.MyTemporal, "90")       == 90.0
            @test VC.mytryparse(VC.MyTemporal, "00:01:30") == 90.0   # clock converted to seconds
            @test VC.mytryparse(VC.MyTemporal, "garbage")  === nothing
        end
        @testset "NTuple{2,Int}: accepted forms and rejects" begin
            @test VC.mytryparse(NTuple{2, Int}, "(7,10)")      == (7, 10)
            @test VC.mytryparse(NTuple{2, Int}, "[250, 1]")    == (250, 1)   # bracket form
            @test VC.mytryparse(NTuple{2, Int}, "250,1")       == (250, 1)   # bare form
            @test VC.mytryparse(NTuple{2, Int}, "  250 , 1  ") == (250, 1)   # surrounding whitespace
            @test VC.mytryparse(NTuple{2, Int}, "1,2,3")       === nothing   # not a 2-tuple
            @test VC.mytryparse(NTuple{2, Int}, "abc")         === nothing
            @test VC.mytryparse(NTuple{2, Int}, "(10000000000000000000,1)") === nothing  # >Int64 overflows -> nothing, not a throw
        end
        @testset "type defaults to video when column absent or empty" begin
            @test VC.parse_row((file = "x.mp4", extrinsic = "00:00:01"))[:type] == "video"
            @test VC.parse_row((type = missing, file = "x.mp4"))[:type] == "video"
        end
    end
end
