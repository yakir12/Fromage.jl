...
rectify(file) = ApriltagRectification(;
...
@testset "AprilTag pipeline, stage by stage" begin
    ...
    @testset "the fixture itself is analytic" begin
        ...
        ground = apriltag_ground()
        pose = k -> drone_pose(
            dx = 40sin(2π * (k - 1) / 9), dy = 30cos(2π * (k - 1) / 7),
    ...
    @testset "stage 4 — a stationary camera reproduces the intended track exactly" begin
        ...
        @test maximum(norm(f.refpos[k] - f.v.expected_ref(k)) for k in 1:NFRAMES) < 1.0e-6
    end
    ...
    @testset "stage 6 — altitude (scale)" begin
        check_flight(
            dir, "scale", k -> drone_pose(
                zoom = 1 + 0.25sin(2π * (k - 1) / 15),
                cx = CENTRE, cy = CENTRE
            )
        )
    end
    ...
    @testset "stage 6 — yaw" begin
        check_flight(
            dir, "yaw", k -> drone_pose(
                yaw = deg2rad(25sin(2π * (k - 1) / 15)),
                cx = CENTRE, cy = CENTRE
            )
        )
    end
    ...
    @testset "stage 6 — skew" begin
        check_flight(
            dir, "shear", k -> drone_pose(
                shear = 0.18sin(2π * (k - 1) / 15),
                cx = CENTRE, cy = CENTRE
            )
        )
    end
