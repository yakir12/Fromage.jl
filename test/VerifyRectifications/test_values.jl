@testset "value ranges & imputation" begin
    ...
    @testset "intrinsic window" begin
        ...
        @test flagged(
            check([checkerboardrow(intrinsic_start = "-1", intrinsic_stop = "00:00:04")]),
            1, "intrinsic_start must be larger than or equal to zero"
        )
        @test flagged(
            check([checkerboardrow(intrinsic_start = "00:00:04", intrinsic_stop = "00:00:01")]),
            1, "intrinsic_start must come before intrinsic_stop"
        )
        # a window that runs past the end of the video is caught
        @test flagged(
            check([checkerboardrow(intrinsic_start = "00:00:01", intrinsic_stop = "00:10:00")]),
