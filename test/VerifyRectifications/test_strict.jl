@testset "strict mode & issue report" begin
    ...
    @testset "issue report is printed with no trailing separator" begin
        ...
        @test occursin("row 1 (calibration_id: s): pixel_width must be larger than zero", out)
    ...
    @testset "a blank calibration_id falls back to the plain row label" begin
        _, out = load_capturing([uniformrow(calibration_id = missing, pixel_width = -1)])
    ...
    @testset "an id failure aborts before any video is opened (#121)" begin
        rows = [uniformrow(calibration_id = "a", file = ART.corrupt),
                uniformrow(calibration_id = "b"),
                uniformrow(calibration_id = "b")]
        _, out = capturing() do
            try
