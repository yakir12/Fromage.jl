...
#
# `required` is what the detector cannot work without; `groupcols` is what makes two rows the same
# question. They are deliberately two lists, not one: a pass may group on a column it does not
# require (an imputed one, say), and such a row must form a group of its own rather than be dropped.
#
...
function detect_per_group!(df::AbstractDataFrame, required, groupcols, desc, detect, flag!;
        progress = true)
    usable = subset(dropmissing(df, required; view = true), :issues => ByRow(isempty); view = true)
    gs = groupby(usable, groupcols)
    ks = collect(keys(gs))
    issues = @showprogress desc = desc enabled = progress tmap(detect, ks)
