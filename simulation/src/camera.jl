...
"""
const REFERENCE_FRAME = (1920, 1080)
...
in [`REFERENCE_FRAME`](@ref) and scaled by `min(sar, 1)` into this `sar`'s display frame. `k` holds
the radial coefficients `(k1, k2, k3)` of `1 + k1 r² + k2 r⁴ + k3 r⁶`, and `sar` is the sample
aspect ratio.
...
function Camera(; position, target, f, principal_point, k, sar)
    f > 0 || throw(ArgumentError("f must be positive, got $f"))
    sar > 0 || throw(ArgumentError("sar must be positive, got $sar"))
    width, height = sar ≥ 1 ? (round(Int, REFERENCE_FRAME[1] / sar), REFERENCE_FRAME[2]) :
        (REFERENCE_FRAME[1], round(Int, REFERENCE_FRAME[2] * sar))
...
slope(k, r) = 1 + 3k[1] * r^2 + 5k[2] * r^4 + 7k[3] * r^6
...
function fold_radius(k)
    ...
    for i in 1:round(Int, MAX_RADIUS / step)
        slope(k, i * step) > 0 && continue
        return bisect(r -> slope(k, r) > 0, (i - 1) * step, i * step)
    end
    return Inf
end
